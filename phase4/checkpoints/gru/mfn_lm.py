...
class MyelinFatigueDiagCell(nn.Module):
    ...
    def forward(self, e, h_lam, h_psi, phi_lam, phi_psi):
        ...
        alpha_psi = torch.sigmoid(self.decay_psi_scale * e + self.decay_psi_bias) ** self.beta
...
class MFNDiagForCausalLM(PreTrainedModel, GenerationMixin):
...
class GRUForCausalLM(PreTrainedModel, GenerationMixin):
...
class DenseMFNForCausalLM(PreTrainedModel, GenerationMixin):
