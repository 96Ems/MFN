...
$\alpha_N \ge 0.076$ (published transformer value) requires
$L_{P_4} \le 2.4013 \times (2.612/1.153)^{-0.076} \approx 2.30$. This
prediction was written to \texttt{phase5/scaling.md} before the run.
\textbf{Interim epoch-1 observation (written after the fact, below the
bar):} the learning-rate effect is confirmed---epoch-1 validation loss is
$2.8317$ (ppl 16.97) at $10^{-3}$ versus $3.0541$ (ppl 21.20) for the same
architecture at $5\times10^{-4}$, a $-0.22$ nat gain at identical epoch and
data order. The remaining budget to the $\alpha_N$ bar is $-0.53$ nat over
epochs 2--4 ($\approx -0.18$/epoch), steeper than the $5\times10^{-4}$
trajectory's per-epoch gain ($-0.26$ at epoch 2); the prediction stands
until the run ends.
