...
$\alpha_D$-range of transformers. We preregister the falsifiable bar for the
parameter exponent $\alpha_N$ against the published $\alpha_N = 0.076$.
\end{abstract}
...
MFN 3L$_{1e-3}$ & $[192,128,96]$ & 1 & 2\,612\,384 & $10^{-3}$, 4 ep --- ep1 val 2.8317 (running) \\
...
\subsection{Preregistered falsifiable prediction}
The parameter exponent $\alpha_N$ will be estimated from the matched pair
$P_0$ (MFN 1L, 1\,153\,440) and $P_4$ (MFN 3L$_{1e-3}$, 2\,612\,384), both
at $10^{-3}$ for 4 epochs. With $L(N) = A N^{-\alpha_N}$, the bar
$\alpha_N \ge 0.076$ (Kaplan et al., non-embedding params) requires
$L_{P_4} \le 2.4013 \times (2.612/1.153)^{-0.076} \approx 2.26$ on total
parameters, and $\le 2.4013 \times (1.826/0.564)^{-0.076} \approx 2.20$ on
non-embedding parameters ($V\!=\!4096$ embeddings removed; ratio $3.24\times$).
Proof in \texttt{phase5/proofs/proof\_A3\_phi\_and\_bar.py} (and
\texttt{proof\_A1\_sigmoid\_beta.py} for the $\beta\,x^{\beta-1}$ cliff). This prediction was written to
\texttt{phase5/scaling.md} before the run; the $2.30$ printed in v0.2--v0.4 was
arithmetic error ($0.044$ nat high, corresponds to $\alpha\approx0.053$).
\textbf{Interim observations:} epoch-1 val $2.8317$ (ppl 16.97) at $10^{-3}$
vs $3.0541$ (ppl 21.20) at $5\times10^{-4}$ ($-0.22$); epoch-2 val $2.6017$
(ppl 13.49) vs $2.7945$ at $5\times10^{-4}$ ($-0.19$). Remaining budget to the
total-param bar $2.26$ is $-0.58$ nat over epochs 3--4 ($\approx-0.29$/ep) and
to the non-embedding bar $2.20$ is $-0.64$ nat; the $5\times10^{-4}$
trajectory's ep1$\to$ep2 gain was $-0.26$. The bar stands until the run ends
(expected $\sim$17:40).
...
\paragraph{Axis N (parameters).} One matched pair is running (the
falsifiable bar above). Two-point fits are fragile; the reconciliation
...
three collapses, is identified and eliminated. The parameter exponent
$\alpha_N$ is being measured now with a preregistered bar against the
published transformer value. If MFN's exponents match or exceed the
published ones at equal protocol, the ``scaling barrier" is not a law of
nature but a property of inductive biases---and fatigue-driven multi-scale
recurrence is a candidate for shifting it.
